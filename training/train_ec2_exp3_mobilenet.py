"""
Experiment 3: MobileNetV3 + LSTM — EC2 (local data)

Direct counterpart to train_ec2_s3.py but using MobileNetV3LSTM instead of CNN_LSTM,
and reading data directly from local disk on EC2 (no S3 download).
Timing instrumentation, metric logging, and output format are identical to
train_ec2_s3.py so results can be directly compared in Experiment 3.

Usage:
    python train_ec2_mobilenetv3.py

Results saved to outputs/experiment3/
"""
...
DATA_ROOT = os.path.expanduser("~/Cloud-based-SLR/data_skeleton")
...
for run in range(NUM_RUNS):
    ...
    print(f"\n===== RUN {run+1}/{NUM_RUNS} =====")
    run_id = f"mobilenetv3_ec2_{run}_{int(time.time())}"
