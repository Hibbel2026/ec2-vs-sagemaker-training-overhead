...
# ===== SETTINGS =====
#
# BATCH_SIZE is intentionally kept at 4 to match Experiment 2 (ResNet50 baseline).
# ResNet101 is ~75% larger than ResNet50. On an A10G (24 GB) with SEQUENCE_LENGTH=16
# this should still fit, but if you hit an OOM error reduce BATCH_SIZE to 2 here
# and document the change in your thesis.

DATA_ROOT = "data_skeleton"
...
IMG_SIZE             = 224
SEQUENCE_LENGTH      = 16
BATCH_SIZE           = 4        # see OOM note above
EPOCHS               = 70
...
for run in range(NUM_RUNS):
    ...
    print(f"\n===== RUN {run+1}/{NUM_RUNS} =====")
    run_id = f"resnet101_ec2_{run}_{int(time.time())}"
