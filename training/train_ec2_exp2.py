...
S3_DATA_PREFIX       = "data_skeleton"
LOCAL_CACHE_DIR      = "/tmp/data_skeleton"
...
NUM_RUNS             = 5   # 3-5 per platform per spec
...
def download_from_s3(bucket, prefix, local_dir):
    """
    Download data_skeleton/ from S3 to local EC2 disk once before training.
    Data is shared across all runs — download time is NOT included in
    per-run training cost.
    """
    if os.path.exists(local_dir):
        print(f"Data already cached at {local_dir}, skipping download.")
        return 0.0
    ...
    for page in pages:
        for obj in page.get("Contents", []):
            ...
            local_path = os.path.join(
                local_dir, os.path.relpath(s3_key, prefix)
            )
...
s3_download_time = download_from_s3(S3_BUCKET, S3_DATA_PREFIX, LOCAL_CACHE_DIR)

TRAIN_DIR = os.path.join(LOCAL_CACHE_DIR, "train")
VAL_DIR   = os.path.join(LOCAL_CACHE_DIR, "val")
TEST_DIR  = os.path.join(LOCAL_CACHE_DIR, "test")
...
for run in range(NUM_RUNS):
    ...
    for epoch in range(EPOCHS):
        ...
        # Per-component accumulators (seconds, converted to ms at log time)
        ...
        # All timing in ms/epoch as per spec
        epoch_logs.append({
            "epoch":                    epoch + 1,
    ...
    results = {
        ...
        "s3_download_time_sec":         s3_download_time,
        "run_id":                       run_id,
